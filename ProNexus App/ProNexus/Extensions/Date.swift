...
// https://github.com/melvitax/DateHelper/blob/master/Sources/DateHelper.swift
...
extension Date {
    static var currentTimeInMilliSeconds: Int64{
...
func getTimeAgo() -> String {
    ...
    let interval = Calendar.current.dateComponents([.year, .month, .day, .weekOfMonth, .hour, .minute], from: self, to: Date())
    ...
    if let year = interval.year, year > 0 {
        ...
        return month == 1 ? "\(month)" + " " + "tháng trước" :
            "hơn \(month)" + " " + "tháng trước"
    } else if let weekOfMonth = interval.weekOfMonth, weekOfMonth > 0 {
        return weekOfMonth == 1 ? "\(weekOfMonth)" + " " + "tuần trước" :
            "hơn \(weekOfMonth)" + " " + "tuần trước"
    }  else if let day = interval.day, day > 0 {
        return day == 1 ? "\(day)" + " " + "ngày trước" :
            "hơn \(day)" + " " + "ngày trước"
    } else if let hour = interval.hour, hour > 0 {
        return hour == 1 ? "\(hour)" + " " + "giờ trước" :
            "hơn \(hour)" + " " + "giờ trước"
    }  else if let minute = interval.minute, minute > 0 {
        return minute == 1 ? "\(minute)" + " " + "phút trước" :
            "hơn \(minute)" + " " + "phút trước"
    }  else {
        return "vừa xong"
